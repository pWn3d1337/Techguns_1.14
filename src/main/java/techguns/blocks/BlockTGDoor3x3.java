...
public class BlockTGDoor3x3<T extends Enum<T> & IStringSerializable> extends GenericBlock {
	...
	protected BlockPos findMaster(IBlockAccess w, BlockPos pos, BlockState state) {
		if(state.getValue(MASTER)) {
			return pos;
		}
		Vec3i[] offsets = pos_x;
	...
	public boolean isStateOpen(BlockState state) {
	...
	public AxisAlignedBB getBoundingBox(BlockState state, IBlockAccess source, BlockPos pos) {
		...
		if(!isStateOpen(state)) {
			...
			BlockState masterstate=getActualState(source.getBlockState(master), source, master);
	...
	protected AxisAlignedBB getOpenBBforTypeMetal(BlockState state, BlockPos pos, BlockPos master) {
	...
	protected AxisAlignedBB getOpenBBforTypeHangarUp(BlockState state, BlockPos pos, BlockPos master) {
		if(state.getValue(MASTER) || master.getY()>= pos.getY()) {
			return NO_COLLIDE;
		} else {
			if(!state.getValue(ZPLANE)) {
	...
	protected AxisAlignedBB getOpenBBforTypeHangarDown(BlockState state, BlockPos pos, BlockPos master) {
		if(state.getValue(MASTER) || master.getY()<= pos.getY()) {
			return NO_COLLIDE;
		} else {
			if(!state.getValue(ZPLANE)) {
	...
	protected AxisAlignedBB getBBforPlane(BlockState state) {
		if(state.getValue(ZPLANE)) {
			return BB_X;
		} else {
			return BB_Z;
	...
	public void toggleState(World w, BlockPos masterPos, boolean checkNeighbours) {
		BlockState masterstate = w.getBlockState(masterPos);
	...
	public void checkOpenNeighbours(World w, BlockPos masterPos, BlockState masterstate, Door3x3TileEntity tile) {
	...
	protected boolean checkBlockAndToggleState(World w, BlockPos pos, BlockState masterstate) {
	...
	protected boolean isMatchingDoorType(BlockState master, BlockState other) {
	...
	public void setOpenedStateForBlock(World w, BlockPos p, EnumDoorState doorstate) {
		...
		if(state.getBlock()==this) {
			BlockState newstate = state.withProperty(STATE,doorstate);
	...
	public boolean canPlaceDoor(World worldIn, BlockPos pos, Direction lookdirection) {
		int xmin = 0;
		int xmax = 1;
		int zmin = 0;
		int zmax = 1;
	...
	protected boolean checkOpenForNearPlayers(World w, Door3x3TileEntity door, BlockPos masterPos) {
		...
		List<PlayerEntity> nearbyPlayers = w.getEntitiesWithinAABB(PlayerEntity.class, bb);
		
		for (PlayerEntity ply : nearbyPlayers) {
