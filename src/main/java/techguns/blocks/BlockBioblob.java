...
public class BlockBioblob extends GenericBlock {
	...
	public BlockBioblob(String name) {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(FACING_ALL, Direction.DOWN).withProperty(SIZE, 0));
	}

	
	protected AxisAlignedBB getBB(BlockState state, IBlockAccess w, BlockPos pos) {
		int size=2;
	...
	protected AxisAlignedBB getBBForRota(int W, int H, Direction facing) {
	...
	public BlockState getStateFromMeta(int meta) {
		return this.getDefaultState().withProperty(FACING_ALL, Direction.VALUES[meta]);
	}
