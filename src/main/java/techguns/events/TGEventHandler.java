...
public class TGEventHandler {
	...
	public static void onMouseEvent(MouseEvent event) {
		if (event.getButton()!=-1){
			...
			ClientPlayerEntity ply = Minecraft.getMinecraft().player;
	...
	public static void rightClickEvent(PlayerInteractEvent.RightClickBlock event) {
		...
		if (cancel) {
			...
			PlayerEntity ply = event.getEntityPlayer();
	...
	public static void onRenderLivingEventPre(RenderLivingEvent.Pre event){
		if (event.getEntity() instanceof PlayerEntity) {
			PlayerEntity ply = (PlayerEntity) event.getEntity();
		 	...
		 	if(!stack.isEmpty() && stack.getItem() instanceof GenericGun && ((GenericGun) stack.getItem()).hasBowAnim()){
		 		...
		 		if (mdl instanceof ModelPlayer) {
			 		...
			 		if (ply.getPrimaryHand()== HandSide.RIGHT) {
	...
	public static void OnLivingAttack(LivingAttackEvent event){
		...
		} else if ( (event.getSource() == DamageSource.LAVA || event.getSource()==DamageSource.ON_FIRE || event.getSource()==DamageSource.IN_FIRE) && event.getEntityLiving() instanceof PlayerEntity) {
			float bonus = GenericArmor.getArmorBonusForPlayer((PlayerEntity) event.getEntityLiving(), TGArmorBonus.COOLING_SYSTEM,event.getEntityLiving().world.getTotalWorldTime()%5==0);
	...
	public static void onBreakEventHigh(BreakSpeed event){
		...
		if(!item.isEmpty() && item.getItem() instanceof GenericGunMeleeCharge) {
			...
			if(g.getMiningRadius(item)>0) {	
				Direction sidehit = g.getSideHitMining(ply.world, ply);
				...
				if (sidehit!=null) {
					...
					for (BlockPos p: blocks) {
						BlockState s = ply.world.getBlockState(p);
	...
	public static void onLivingDeathEvent(LivingDeathEvent event){
		...
		if(!entity.world.isRemote){
			...
			if(entity instanceof PlayerEntity){
				TGExtendedPlayer tgplayer = TGExtendedPlayer.get((PlayerEntity) event.getEntityLiving());
				tgplayer.foodleft=0;
	...
	public static void onRenderHand(RenderHandEvent event) {
		float t = 1.0f;
		PlayerEntity ply = ClientProxy.get().getPlayerClient();
...
//		states.restore();
			...
			if(!hasInvalid && hasGun && hasAmmo) {
				...
				items.forEach(i -> {
					...
					if(amount>0 && !event.player.world.isRemote) {
						ItemStack it = i.copy();
						it.setCount(amount);
						event.player.world.spawnEntity(new ItemEntity(event.player.world, event.player.posX, event.player.posY, event.player.posZ, it));
					}		
					});
			}
	...
	public static Method Block_getSilkTouchDrop = ReflectionHelper.findMethod(Block.class, "getSilkTouchDrop", "func_180643_i", BlockState.class);

	@SubscribeEvent
	public static void onBlockDrops(HarvestDropsEvent event) {
		PlayerEntity ply = event.getHarvester();
		if(ply!=null) {
			ItemStack stack = ply.getHeldItemMainhand();
	...
	public static void onBlockBreakEvent(BlockEvent.BreakEvent event) {
		PlayerEntity ply = event.getPlayer();
		if(ply!=null) {
			ItemStack stack = ply.getHeldItemMainhand();
			if(!stack.isEmpty() && stack.getItem() instanceof MiningDrill && ply.isSneaking()) {
	...
	public static void MilitaryCrateDrops(HarvestDropsEvent event) {
		...
		if(state.getBlock()==TGBlocks.MILITARY_CRATE && !event.isSilkTouching() && !event.getWorld().isRemote) {
			...
			if (ply!=null) {
				...
				LootContext lootcontext = new LootContext.Builder((ServerWorld) ply.world).withLuck(fortune).withPlayer(ply).build();
	...
	public static void onEntityConstruction(EntityConstructing event) {
		...
		if(event.getEntity() instanceof LivingEntity) {
			LivingEntity elb = (LivingEntity) event.getEntity();
