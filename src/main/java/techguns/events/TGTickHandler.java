...
public class TGTickHandler {
	...
	static {
		...
		ITEMFOOD_onFoodEaten = ObfuscationReflectionHelper.findMethod(ItemFood.class, "func_77849_c", void.class, ItemStack.class, World.class, PlayerEntity.class);
	}
						...
						if (cp.keyFirePressedMainhand) {
							...
							if (props.getFireDelay(Hand.MAIN_HAND) <= 0) {
								if (gun instanceof GenericGunCharge && ((GenericGunCharge)gun).getLockOnTicks() > 0 && props.lockOnEntity != null && props.lockOnTicks > ((GenericGunCharge)gun).getLockOnTicks()) {
									TGPackets.network.sendToServer(new PacketShootGunTarget(gun.isZooming(), Hand.MAIN_HAND, props.lockOnEntity));
									gun.shootGunPrimary(stack, event.player.world, event.player, gun.isZooming(), Hand.MAIN_HAND, props.lockOnEntity);
								}else {
									TGPackets.network.sendToServer(new PacketShootGun(gun.isZooming(), Hand.MAIN_HAND));
									gun.shootGunPrimary(stack, event.player.world, event.player, gun.isZooming(), Hand.MAIN_HAND, null);
								}
					...
					if (!stackOff.isEmpty() && stackOff.getItem() instanceof IGenericGun && ((IGenericGun) stackOff.getItem()).isShootWithLeftClick()) {
						if (cp.keyFirePressedOffhand) {
							...
							if (props.getFireDelay(Hand.OFF_HAND) <= 0) {
								...
								TGPackets.network.sendToServer(new PacketShootGun(gun.isZooming(), Hand.OFF_HAND));
								gun.shootGunPrimary(stackOff, event.player.world, event.player, gun.isZooming(), Hand.OFF_HAND, null);
							}
							if (gun.isSemiAuto()) {
		...
		TGExtendedPlayer props = TGExtendedPlayer.get(event.player);
		 ...
		 if (event.phase == Phase.START){
			 ...
			 if(nightvision>0.0f && enabled){
				 if(!event.player.world.isRemote){
					 event.player.addPotionEffect(new EffectInstance(Effects.NIGHT_VISION, 220, 0,false,false));
				 }
			 }
			 ...
			 if (!TGConfig.disableAutofeeder && event.player.getFoodStats().getFoodLevel()<=19){
				 ...
				 if (props!=null){
					 ...
					 if (props.foodleft>0){
						 ...
						 if (!event.player.world.isRemote){
							 TGPackets.network.sendTo(new PacketTGExtendedPlayerSync(event.player,props,true), (ServerPlayerEntity) event.player);
						 }
					 } else {
			 ...
			 if(TGRadiationSystem.isEnabled() && event.side==Side.SERVER && event.player.world.getTotalWorldTime()%TGRadiationSystem.INVENTORY_RADIATION_INVERVALL==0) {
				 ...
				 if(maxrad>0) {
					 event.player.addPotionEffect(new EffectInstance(TGRadiationSystem.radiation_effect,TGRadiationSystem.INVENTORY_RADIATION_INVERVALL,maxrad-1,false,false));
				 }
			 }
			 ...
			 if(event.side==Side.SERVER && event.player.world.getTotalWorldTime()%20==0) {
				 if(props.radlevel>=TGRadiationSystem.MINOR_POISONING && props.radlevel < TGRadiationSystem.SEVERE_POISONING ) {
					 event.player.addPotionEffect(new EffectInstance(Effects.HUNGER,30,0,false,false));
					 
				} else if (props.radlevel>=TGRadiationSystem.SEVERE_POISONING) {
					event.player.addPotionEffect(new EffectInstance(Effects.HUNGER,30,1,false,false));
					event.player.addPotionEffect(new EffectInstance(Effects.MINING_FATIGUE,30,1,false,false));
					event.player.addPotionEffect(new EffectInstance(Effects.WEAKNESS,30,1,false,false));
				} 
				 
				if(props.radlevel>=TGRadiationSystem.LETHAL_POISONING) {
					event.player.addPotionEffect(new EffectInstance(Effects.NAUSEA,30,1,false,false));
					event.player.addPotionEffect(new EffectInstance(Effects.SLOWNESS,30,1,false,false));
...
//			 }
	...
	public static void TickParticleSystems(ClientTickEvent event) {
		if(event.phase==Phase.END) {
			...
			if(w!=null) {
				w.<LivingEntity>getEntities(LivingEntity.class, new Predicate<LivingEntity>() {
					@Override
				...
				w.getPlayers(PlayerEntity.class, new Predicate<PlayerEntity>() {
					@Override
	...
	public static void onRenderTick(RenderTickEvent event) {
		...
		if (event.phase==Phase.START) {
			...
			PlayerEntity player = cp.getPlayerClient();
			if (player != null){
