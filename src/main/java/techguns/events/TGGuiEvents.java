...
public class TGGuiEvents extends AbstractGui {
	...
	public void onRenderGameOverlyEvent(RenderGameOverlayEvent event){
		...
		if(props!=null && props.showTGHudElements){
			...
			if(props.radlevel>0) {
				...
				EffectInstance currentRad = ply.getActivePotionEffect(TGRadiationSystem.radiation_effect);
				if(currentRad!=null) {
	...
	private int getAmmoCountOfStack(ItemStack ammoitem, GenericGun gun, PlayerEntity ply, TGExtendedPlayer props) {
		int count = InventoryUtil.countItemInInv(ply.inventory.mainInventory, ammoitem, 0, ply.inventory.mainInventory.size());
		count += InventoryUtil.countItemInInv(props.tg_inventory.inventory, ammoitem, TGPlayerInventory.SLOTS_AMMO_START, TGPlayerInventory.SLOTS_AMMO_END+1);
	...
	private void drawGunAmmoCount(Minecraft mc, ScaledResolution sr, GenericGun gun, ItemStack item, PlayerEntity ply, TGExtendedPlayer props, int offsetY) {
...
//	            }
				...
				if(gun.isZooming()) {
					...
					if(gun.getCrossHairStyle()!=EnumCrosshairStyle.VANILLA) {
		                ...
		                Minecraft.getMinecraft().getTextureManager().bindTexture(AbstractGui.ICONS);
				}
			}
