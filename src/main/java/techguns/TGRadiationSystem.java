...
public class TGRadiationSystem implements ITGInitializer {
	...
	public static Effect RAD_POTION;
	public static Effect RAD_RESISTANCE_POTION;
	public static Effect RAD_REGENERATION_POTION;
	public static Effect RAD_POTION_SEVERE;
	...
	public static void applyRadToEntities(World world, double posX, double posY, double posZ, double radius, int duration, int strength, double inner_radius, int strength_outer) {
		...
		world.getEntitiesWithinAABB(LivingEntity.class ,bb, RADIATION_TARGETS).forEach(e -> {
			...
			if (distance < radius) {
				...
				LivingEntity elb = (LivingEntity) e; //RADIATION_TARGETS only applies to EntityLivingBase
				elb.addPotionEffect(new EffectInstance(TGRadiationSystem.radiation_effect, duration,
						str, true, true));
