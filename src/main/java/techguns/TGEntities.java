...
public class TGEntities implements ITGInitializer {
	...
	public void postInit(FMLPostInitializationEvent event) {
		...
		if(TGConfig.spawnWeightTGOverworld>0) {
			EntityRegistry.addSpawn(TGDummySpawn.class, TGConfig.spawnWeightTGOverworld, 1, 3, EntityClassification.MONSTER,overworldBiomes.toArray(new Biome[overworldBiomes.size()]));
		}
		if(TGConfig.spawnWeightTGNether>0) {
			EntityRegistry.addSpawn(TGDummySpawn.class, TGConfig.spawnWeightTGNether, 1, 3, EntityClassification.MONSTER,netherBiomes.toArray(new Biome[netherBiomes.size()]));
		}
	}
