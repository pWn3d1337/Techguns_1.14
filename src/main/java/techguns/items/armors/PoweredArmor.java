...
public class PoweredArmor extends GenericArmorMultiCamo {
	...
	public PoweredArmor(String unlocalizedName, TGArmorMaterial material,
                        String[] textureNames, EquipmentSlotType type, ArmorPowerType powerType, int maxpower) {
	...
	public int applyPowerConsumptionOnAction(TGArmorBonus type, PlayerEntity player){
		...
		if (type==TGArmorBonus.JUMP){
			if (this.armorType== EquipmentSlotType.LEGS){ //Legs
			...
			if (this.armorType== EquipmentSlotType.FEET){
				...
				if (InventoryUtil.consumeAmmoPlayer(player,powerChest.battery)) {
    				...
    				if (powerChest.battery_empty!=null){
    					...
    					if(amount >0 && !player.world.isRemote){
    						player.world.spawnEntity(new ItemEntity(player.world, player.posX, player.posY, player.posZ, TGItems.newStack(powerChest.battery_empty, amount)));
    					}
