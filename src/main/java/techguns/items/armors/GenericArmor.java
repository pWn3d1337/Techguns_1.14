...
public class GenericArmor extends ArmorItem implements ISpecialArmor , IItemTGRenderer{
	...
	public GenericArmor(String unlocalizedName, TGArmorMaterial material, String textureName, EquipmentSlotType type) {
		this(Techguns.MODID, unlocalizedName, material, textureName, type);
	}
	
	public GenericArmor(String modid, String unlocalizedName, TGArmorMaterial material, String textureName, EquipmentSlotType type) {
	...
	public static float getArmorBonusForPlayer(PlayerEntity ply, TGArmorBonus type, boolean consumePower){
		float bonus = 0.0f;
	...
	private static float getBonusForSlot(ItemStack stack, TGArmorBonus type, boolean consumePower, PlayerEntity ply) {
		float bonus=0;
		if(!stack.isEmpty()){
	...
	private String getDoubleTexture(){
		return this.modid + ":textures/armor/" + this.textureName + "_" + (this.armorType == EquipmentSlotType.LEGS ? "2" : "1") + ".png";
	}
	...
	public ModelBiped getArmorModel(LivingEntity entityLiving, ItemStack itemStack, EquipmentSlotType armorSlot, ModelBiped _default) {
		...
		if(this.armorModel!=null) {
			...
			model.bipedBody.showModel = armorSlot == EquipmentSlotType.CHEST || armorSlot == EquipmentSlotType.LEGS;
	...
	protected void addArmorvaluesInformation(ItemStack item, List list){
		...
		EquipmentSlotType slot = this.armorType;
	...
	public void damageArmor(LivingEntity entity, ItemStack stack,
		...
		if (!(entity instanceof ServerPlayerEntity) || !((PlayerEntity)entity).capabilities.isCreativeMode)
        {
