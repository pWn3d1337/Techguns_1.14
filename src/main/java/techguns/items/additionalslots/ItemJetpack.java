...
public class ItemJetpack extends ItemTGSpecialSlotAmmo {
	...
	public void onPlayerTick(ItemStack item, PlayerTickEvent event) {
		...
		if (event.player.world.isRemote) {
			...
			PlayerEntity self = Techguns.proxy.getPlayerClient();// Minecraft.getMinecraft().thePlayer;
	...
	protected void applyBoostToPlayer(PlayerEntity ply, ItemStack item) {
	...
	protected void descendPlayer(PlayerEntity ply, ItemStack item) {
	...
	protected void hoverPlayer(PlayerEntity ply, ItemStack item, boolean forwardPressed) {
		if (ply.motionY > 0) {
