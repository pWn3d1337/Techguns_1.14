...
public class GenericGun extends GenericItem implements IGenericGun, IItemTGRenderer, ICamoChangeable {
	...
	public static boolean getDoBlockDamage(LivingEntity elb) {
		...
		if (elb instanceof PlayerEntity){
			TGExtendedPlayer caps = TGExtendedPlayer.get((PlayerEntity) elb);
    		...
    		if (ammo>0) {
		    	...
		    	if (firedelay <=0) {
			        ...
			        if (!world.isRemote)
			        {
			        	...
			        	if(hand== Hand.MAIN_HAND && player.isActiveItemStackBlocking()) {
			        	...
			        	if ((hand == Hand.MAIN_HAND && player.getPrimaryHand() == HandSide.RIGHT) || (hand == Hand.OFF_HAND && player.getPrimaryHand() == HandSide.LEFT)) {
    				...
    				Arrays.stream(this.ammoType.getEmptyMag()).forEach( e -> {
    					if (!e.isEmpty()){
        					...
        					if(amount >0 && !world.isRemote){
        						player.world.spawnEntity(new ItemEntity(player.world, player.posX, player.posY, player.posZ, TGItems.newStack(e, amount)));
        					}
        				}
    				});
	...
	protected void shootGun(World world, LivingEntity player, ItemStack itemstack, float accuracybonus, float damagebonus, int attackType, Hand hand, EnumBulletFirePos firePos, Entity target){
		...
		if (!world.isRemote){
	    	...
	    	TGPackets.network.sendToAllAround(new GunFiredMessage(player,msg_recoiltime,msg_muzzleflashtime,attackType,checkRecoil,hand== Hand.OFF_HAND), TGPackets.targetPointAroundEnt(player, 100.0f));
		}
    	//
	 ...
	 * @param tags
	 */
	protected void addInitialTags(CompoundNBT tags) {
	...
	@Override
	public boolean onLeftClickEntity(ItemStack stack, PlayerEntity player, Entity targetEntity) {
				...
				if (!targetEntity.hitByEntity(player)) {
					...
					if (f > 0.0F || f1 > 0.0F) {
						...
						if (player.isSprinting() && flag) {
							player.world.playSound((PlayerEntity) null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_KNOCKBACK,
						...
						boolean flag2 = flag && player.fallDistance > 0.0F && !player.onGround && !player.isOnLadder() && !player.isInWater()
								&& !player.isPotionActive(Effects.BLINDNESS) && !player.isRiding() && targetEntity instanceof LivingEntity;
						...
						if (flag5) {
							...
							if (i > 0) {
								if (targetEntity instanceof LivingEntity) {
									((LivingEntity) targetEntity).knockBack(player, (float) i * 0.5F, (double) MathHelper.sin(player.rotationYaw * 0.017453292F),
							...
							if (flag3) {
								...
								for (LivingEntity entitylivingbase : player.world.getEntitiesWithinAABB(LivingEntity.class,
							...
							if (flag2) {
								player.world.playSound((PlayerEntity) null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_CRIT, player.getSoundCategory(),
										1.0F, 1.0F);
							...
							if (!flag2 && !flag3) {
								if (flag) {
									player.world.playSound((PlayerEntity) null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_STRONG,
											player.getSoundCategory(), 1.0F, 1.0F);
								} else {
									player.world.playSound((PlayerEntity) null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_WEAK,
											player.getSoundCategory(), 1.0F, 1.0F);
								}
							}
							...
							if (targetEntity instanceof LivingEntity) {
								...
								if (player.world instanceof ServerWorld && f5 > 2.0F) {
									...
									((ServerWorld) player.world).spawnParticle(EnumParticleTypes.DAMAGE_INDICATOR, targetEntity.posX,
							...
							player.world.playSound((PlayerEntity) null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_NODAMAGE, player.getSoundCategory(),
									1.0F, 1.0F);
	...
	protected void consumeAmmoOnMeleeHit(LivingEntity elb, ItemStack stack) {
		if(elb instanceof PlayerEntity) {
			PlayerEntity ply = (PlayerEntity) elb;
			if(ply.capabilities.isCreativeMode) return;
		}
		this.useAmmo(stack, 1);
	}
	
	protected void doSweepAttackEffect(PlayerEntity player) {
        if(!player.world.isRemote) {
	...
	protected void playSweepSoundEffect(PlayerEntity player) {
		player.world.playSound((PlayerEntity) null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_SWEEP,
				player.getSoundCategory(), 1.0F, 1.0F);
	}
     ...
     */
    public void fireWeaponFromNPC(LivingEntity shooter, float dmgscale, float accscale) {
    	...
    	if (!shooter.world.isRemote){
    		this.shootGun(shooter.world, shooter, shooter.getHeldItemMainhand(), this.zoombonus*accscale,dmgscale,0, Hand.MAIN_HAND, firePos, null);
    	}
			...
			if (InventoryUtil.consumeAmmoPlayer(player,this.getReloadItem(item))) {
				...
				for (int i=0;i<ammos;i++) {
					if (!this.ammoType.getEmptyMag()[i].isEmpty()){
						...
						if(amount >0 && !world.isRemote){
							player.world.spawnEntity(new ItemEntity(player.world, player.posX, player.posY, player.posZ, TGItems.newStack(this.ammoType.getEmptyMag()[i], amount)));
						}
						...
						if (bulletsBack>0){
							...
							if(amount2 >0 && !world.isRemote){
								player.world.spawnEntity(new ItemEntity(player.world, player.posX, player.posY, player.posZ, TGItems.newStack(this.ammoType.getBullet(this.getCurrentAmmoVariant(item))[i], amount2)));
							}
						}
	...
	public boolean canClickBlock(World world, PlayerEntity player, Hand hand) {
