...
public class Shishkebap extends GenericGunMeleeCharge {
	...
	protected void consumeAmmoOnMeleeHit(LivingEntity elb, ItemStack stack) {
		...
		if(!elb.world.isRemote && !empty && this.getAmmoLeft(stack)<=0) {
			TGPackets.network.sendToDimension(new PacketNotifyAmbientEffectChange(elb, Hand.MAIN_HAND), elb.world.provider.getDimension());
		}
	}
	...
	@Override
	public void onPlayerStoppedUsing(ItemStack item, World world, LivingEntity entityLiving, int timeLeft) {
	}
