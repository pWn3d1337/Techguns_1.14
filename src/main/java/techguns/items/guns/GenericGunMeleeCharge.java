...
public class GenericGunMeleeCharge extends GenericGunCharge implements IGenericGunMelee<GenericGunMeleeCharge> {
	...
	@Override
	public int getExtraMiningLevel(ItemStack stack, String toolClass, PlayerEntity player) {
	...
	public boolean onEntitySwing(LivingEntity entityLiving, ItemStack stack) {
		...
		if (!entityLiving.world.isRemote) {
			if(entityLiving instanceof PlayerEntity){
				PlayerEntity ply = (PlayerEntity) entityLiving;
				if(ply.openContainer!=null){
					if (!(ply.openContainer.getClass() == PlayerContainer.class)){
		...
		if ( this.getCurrentAmmo(stack) > 0) {
			if (entityLiving.world.isRemote) {
				...
				if(entityLiving instanceof PlayerEntity){
					TGExtendedPlayer props = TGExtendedPlayer.get((PlayerEntity)entityLiving);
	...
	public boolean onBlockDestroyed(ItemStack stack, World worldIn, BlockState state, BlockPos pos, LivingEntity entityLiving) {
		if(this.getCurrentAmmo(stack)>0){
			...
			if (entityLiving instanceof PlayerEntity) {
				...
				if(sound!=null) {
					worldIn.playSound((PlayerEntity)entityLiving, pos.getX()+0.5D, pos.getY()+0.5D, pos.getZ()+0.5D, sound, SoundCategory.PLAYERS, melee_sound_volume, 1.0f);
				}
			}
			return true;
		}
	...
	public Direction getSideHitMining(World world, PlayerEntity player) {
	...
	public boolean onBlockStartBreak(ItemStack itemstack, BlockPos pos, PlayerEntity player) {
		if(!player.world.isRemote && !player.isSneaking() && this.getMiningRadius(itemstack)>0) {
			...
			Direction sideHit = this.getSideHitMining(player.world, player);
			if(sideHit!=null) {
				...
				otherblocks.forEach(b -> {
					if(w.isBlockLoaded(b)) {
						BlockState state = w.getBlockState(b);
						if(state.getBlock()!=null && !(state.getBlock()== Blocks.AIR) && state.getPlayerRelativeBlockHardness(player, w, b) > 0f
							...
							ServerPlayerEntity plyMP = (ServerPlayerEntity) player;
							...
							if (xpAmount>=0) {
								...
								if(block.removedByPlayer(state, w, b, player, true)){
									...
									plyMP.connection.sendPacket(new SChangeBlockPacket(w, b));
								}
							}
						}
					}
