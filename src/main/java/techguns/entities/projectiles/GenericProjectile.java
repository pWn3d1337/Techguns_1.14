...
public class GenericProjectile extends Entity implements IProjectile, IEntityAdditionalSpawnData {
	...
	public static final Predicate<Entity> BULLET_TARGETS = Predicates.and(EntityPredicates.CAN_AI_TARGET,
			EntityPredicates.IS_ALIVE, new Predicate<Entity>() {
...
//	}
	...
	public GenericProjectile(World par2World, LivingEntity p, float damage, float speed, int TTL, float spread,
	...
	protected void onHit(RayTraceResult raytraceResultIn) {
		...
		if (raytraceResultIn.entityHit != null && !this.world.isRemote) {
			...
			if (raytraceResultIn.entityHit instanceof LivingEntity) {
				LivingEntity ent = (LivingEntity) raytraceResultIn.entityHit;
				...
				if (dmg > 0.0f) {
					...
					if (src.wasSuccessful()) {
						...
						if (ent instanceof MobEntity) {
							this.setAIRevengeTarget(((MobEntity) ent));
						}
	...
	protected void setAIRevengeTarget(MobEntity ent) {
	 ...
	 * @param ent
	 */
	protected void onHitEffect(LivingEntity ent, RayTraceResult rayTraceResultIn) {
	}
	...
	protected void hitBlock(RayTraceResult raytraceResultIn) {
		...
		BlockState target = this.world.getBlockState(targetPos);
	...
	@Override
	protected void readEntityFromNBT(CompoundNBT tags) {
	...
	@Override
	protected void writeEntityToNBT(CompoundNBT tags) {
