...
public class BioGunProjectile extends GenericProjectile implements IEntityAdditionalSpawnData{
	...
	public BioGunProjectile(World par2World, LivingEntity p, float damage, float speed, int TTL, float spread, float dmgDropStart, float dmgDropEnd, float dmgMin,
	...
	public BioGunProjectile(World worldIn) {
	 ...
	 @Override
	protected void onHitEffect(LivingEntity ent, RayTraceResult rayTraceResult) {
 		ent.addPotionEffect(new EffectInstance(Effects.POISON,100,3,false,true));
	...
	protected void doImpactEffects(Material mat, RayTraceResult rayTraceResult, SoundType sound) {
    	...
    	if (rayTraceResult.typeOfHit == Type.BLOCK) {
    		if (rayTraceResult.sideHit == Direction.UP) {
    			pitch = -90.0f;
    		}else if (rayTraceResult.sideHit == Direction.DOWN) {
    			pitch = 90.0f;
    		}else {
	...
	protected void hitBlock(RayTraceResult mop) {
		 BlockState statehit = this.world.getBlockState(mop.getBlockPos());
    			 ...
    			 if (this.world.isAirBlock(blobPos)){
    				 ...
    				 if(this.blockdamage) {
	    				 ...
	    				 if ( this.shooter instanceof PlayerEntity){
		    				 final BlockEvent.PlaceEvent placeEvent = new BlockEvent.PlaceEvent(BlockSnapshot.getBlockSnapshot(world, blobPos), statehit, (PlayerEntity) this.shooter, Hand.MAIN_HAND);
		    				 MinecraftForge.EVENT_BUS.post(placeEvent);
		    				 canPlace = !placeEvent.isCanceled();
	    				 }
	    				 
	    				 if (canPlace){
	    					 BlockState state = TGBlocks.BIOBLOB.getDefaultState().withProperty(TGBlocks.BIOBLOB.FACING_ALL, mop.sideHit.getOpposite()).withProperty(TGBlocks.BIOBLOB.SIZE, 0);
	    					 int lvl = 0;
