...
public class ReactionChamberTileEntMaster extends MultiBlockMachineTileEntMaster implements ITileEntityFluidTanks, SimpleComponent {
	...
	public AxisAlignedBB getRenderBoundingBox() {
		if(this.isFormed()) {
			...
			Direction left = multiblockDirection.rotateY();
			
			BlockPos other = p.offset(multiblockDirection, 3).offset(left, 2).offset(Direction.UP,4);
			BlockPos first = p.offset(multiblockDirection.getOpposite(),1).offset(left.getOpposite(),2).offset(Direction.DOWN,1);
	...
	public void readClientDataFromNBT(CompoundNBT tags) {
		...
		CompoundNBT inputTankTags = tags.getCompoundTag("inputTank");
	...
	public void writeClientDataToNBT(CompoundNBT tags) {
		...
		CompoundNBT inputTankTags = new CompoundNBT();
	...
	@Override
	public void readOperationFromNBT(CompoundNBT tags) {
	...
	public ITextComponent getDisplayName() {
		return new TranslationTextComponent(Techguns.MODID+".container.reactionchamber", new Object[0]);
	}
	...
	private void mergeOutput(){
		...
		for (int i=0;i<outputs.size();i++){
			...
			if(leftover>0) {
				this.world.spawnEntity(new ItemEntity(this.world, this.pos.getX()+0.5d, this.pos.getY()+0.5d, this.pos.getZ()+0.5d, out));
			}
	...
	protected void explode(int type){
		Direction dir = this.multiblockDirection;
		...
		if (!this.world.isRemote){
			...
			if (type==0){
				...
				blocksToRemove.add(centerPos.offset(Direction.SOUTH));
				blocksToRemove.add(centerPos.offset(Direction.NORTH));
				blocksToRemove.add(centerPos.offset(Direction.WEST));
				blocksToRemove.add(centerPos.offset(Direction.EAST));

			} else if (type==1){
				BlockPos pos = masterPos.toImmutable();
				
				Direction left = dir.rotateY();
			...
			if (type == 0){
				...
				this.world.playSound((PlayerEntity)null, centerPos.getX()+0.5d, centerPos.getY()+0.5d, centerPos.getZ()+0.5d, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 4.0F, (1.0F + (this.world.rand.nextFloat() - this.world.rand.nextFloat()) * 0.2F) * 0.7F);
			...
			if (!content.isEmpty()){
				this.inventory.setStackInSlot(SLOT_INPUT, ItemStack.EMPTY);
				this.world.spawnEntity(new ItemEntity(this.world, centerPos.getX()+0.5d, centerPos.getY()+0.5d, centerPos.getZ()+0.5d, content));
			}
	...
	public void loadTanksFromNBT(CompoundNBT tags) {
		CompoundNBT inputTank = tags.getCompoundTag("inputTank");
	...
	public void needFluidUpdate() {
		if (!this.world.isRemote) {
			...
			if(entry !=null ) {
				try {
					List<ServerPlayerEntity> players = (List<ServerPlayerEntity>) playerChunkMapEntry_Players.get(entry);
