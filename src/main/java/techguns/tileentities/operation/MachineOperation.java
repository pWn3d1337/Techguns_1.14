...
public class MachineOperation {
	...
	protected void writeSubclassDataToNBT(CompoundNBT tags) {
		
	}
	
	
	public void readFromNBT(CompoundNBT compound) {
		CompoundNBT tags = compound.getCompoundTag("operation");
		...
		if(tags.hasKey("InputSize")) {
			...
			ListNBT tagList = tags.getTagList("InputItems", Constants.NBT.TAG_COMPOUND);
		...
		if(tags.hasKey("OutputSize")) {
			...
			ListNBT tagList = tags.getTagList("OutputItems", Constants.NBT.TAG_COMPOUND);
			for (int i=0;i<size;i++) {
				CompoundNBT itemTags = tagList.getCompoundTagAt(i);
		...
		if(tags.hasKey("FluidSizeIn")) {
			...
			ListNBT tagList = tags.getTagList("FluidsIn", Constants.NBT.TAG_COMPOUND);
			for (int i=0;i<size;i++) {
				CompoundNBT fluidTags = tagList.getCompoundTagAt(i);
		...
		if(tags.hasKey("FluidSize")) {
			...
			ListNBT tagList = tags.getTagList("Fluids", Constants.NBT.TAG_COMPOUND);
			for (int i=0;i<size;i++) {
				CompoundNBT fluidTags = tagList.getCompoundTagAt(i);
	...
	protected void readSubClassDataFromNBT(CompoundNBT tags) {
