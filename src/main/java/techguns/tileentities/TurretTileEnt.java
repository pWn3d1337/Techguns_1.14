...
public class TurretTileEnt extends BasicPoweredTileEnt implements ITickable{
	...
	public ITextComponent getDisplayName() {
		return new TranslationTextComponent(Techguns.MODID+".container.turretbase", new Object[0]);
	}
...
//			this.mountedTurret=null;
		...
		this.facing = Direction.getFront(tags.getByte("facing"));
	...
	public void rotateTile(Direction sideHit) {
		if (this.facing!=sideHit && (sideHit== Direction.UP || sideHit== Direction.DOWN)) {
			this.facing = sideHit;
	...
	public void writeClientDataToNBT(CompoundNBT tags) {
		...
		CompoundNBT tagsWeapon = new CompoundNBT();
		this.inventory.getStackInSlot(SLOT_WEAPON).writeToNBT(tagsWeapon);
		
		CompoundNBT tagsArmor = new CompoundNBT();
			...
			for (int i=0;i<emptyMag.length;i++) {
				...
				if (InventoryUtil.consumeAmmo(this.inventory, ammo[i],SLOT_INPUT1,SLOT_INPUT1+INPUTS_SIZE)){
					if (!emptyMag[i].isEmpty()){
						...
						if (!this.world.isRemote){
							this.world.spawnEntity(new ItemEntity(this.world, this.pos.getX(), this.pos.getY()+1, this.pos.getZ(), TGItems.newStack(emptyMag[i], tooMuch)));
						}
					}
