...
public class GrinderTileEnt extends BasicMachineTileEnt {
	...
	public ITextComponent getDisplayName() {
		return new TranslationTextComponent("techguns.container.grinder", new Object[0]);
	}
	...
	protected void finishedOperation() {
		...
		if(!this.world.isRemote) {
			leftover.forEach(s -> {
				if(!s.isEmpty()) {
					this.world.spawnEntity(new ItemEntity(this.world, this.pos.getX()+0.5d, this.pos.getY()+0.5d, this.pos.getZ()+0.5d, s));
				}
			});
		}
	}
