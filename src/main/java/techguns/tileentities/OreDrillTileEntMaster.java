...
public class OreDrillTileEntMaster extends MultiBlockMachineTileEntMaster implements ITileEntityFluidTanks{
	...
	protected Direction drill_direction;
	...
	public void formOreDrill(Direction direction, int engines, int rods, int radius, Direction drill_direction ) {
	...
	public ITextComponent getDisplayName() {
		return new TranslationTextComponent(Techguns.MODID+".container.oredrill", new Object[0]);
	}
	...
	public Direction getDrill_direction() {
	...
	public void readClientDataFromNBT(CompoundNBT tags) {
		...
		if(this.formed) {
			this.drill_direction= Direction.getFront(tags.getByte("drill_direction"));
	...
	protected boolean consumePower(int amount) {
		...
		if (this.fuelBuffer*FACTOR>=amount){
			...
			if (!this.inventory.getStackInSlot(SLOT_FURNACE).isEmpty()){
				fuelValue = FurnaceTileEntity.getItemBurnTime(this.inventory.getStackInSlot(SLOT_FURNACE));
			}
			...
			if (fuelValue > 0){
				...
				if (!container.isEmpty()){
					...
					if (leftover >0 && !this.world.isRemote){
						...
						this.world.spawnEntity(new ItemEntity(this.world, this.pos.getX()+0.5, this.pos.getY()+0.5, this.pos.getZ()+0.5, entityItem));
					}
				}
		...
		if (this.isFormed()) {
			...
			//look for cluster
					...
					if (OreDrillDefinition.isOreCluster(this.world, targetPos))
					{
						...
						BlockState bs = this.world.getBlockState(targetPos);
	...
	public int getClusterSize(int count,BlockPos startPos, ArrayList<BlockPos> visited, OreCluster clusterType, BlockState clusterMeta){

		BlockState targetState = this.world.getBlockState(startPos);
	...
	public void saveTanksToNBT(CompoundNBT tags) {
		...
		CompoundNBT outputTankTags = new CompoundNBT();
	...
	public void loadTanksFromNBT(CompoundNBT tags) {
		...
		CompoundNBT outputTank = tags.getCompoundTag("outputTank");
