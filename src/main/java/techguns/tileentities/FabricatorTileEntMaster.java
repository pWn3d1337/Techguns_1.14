...
public class FabricatorTileEntMaster extends MultiBlockMachineTileEntMaster {
	...
	public ITextComponent getDisplayName() {
		return new TranslationTextComponent(Techguns.MODID+".container.fabricator", new Object[0]);
	}
	...
	public AxisAlignedBB getRenderBoundingBox() {
		if(this.isFormed()) {
			...
			BlockPos other = p.offset(multiblockDirection, 2).offset(left, 2).offset(Direction.UP,2);
			BlockPos first = p.offset(multiblockDirection.getOpposite(),2).offset(left.getOpposite(),1).offset(Direction.DOWN,1);
