...
public class MazeDungeonPath implements IDungeonPath {
	...
	protected Direction startFacing = null;
	...
	public void generatePath() {
		...
		switch (this.rand.nextInt(4)) {
			case 0:
				startX = 0;
				startZ = sZ/2;
				startFacing = Direction.getFacingFromVector(1, 0, 0);
				break;
			case 1:
				startX = sX-1;
				startZ = sZ/2;
			...
			case 2:
				startX = sX/2;
				startZ = 0;
				startFacing = Direction.getFacingFromVector(0, 0, 1);
				break;
			case 3:
			default:
				startX = sX/2;
...
//		this.entrancePath.add(startSegment);
	...
	@Override
	public Direction getEntranceRotation() {
	...
	public void generateSegment(int x, int y, int z, int dir, PathSegment prev) {		
		Direction facing = Direction.getHorizontal(dir);
		...
		if (this.rampPlaced < this.entranceRampLength) {
			...
			Direction nextFacing = Direction.getHorizontal(nextDir);
			...
			if (isWithinBounds(nextPos)) {
				if (dir == nextDir && !segment.isEntrance && !fork && rand.nextFloat() < chanceRamp) { //ramp
					...
					if (seg != null && !seg.isRamp && !seg.isEntrance) {
						...
						seg.setConnection(Direction.getHorizontal(nextDir).getOpposite(), true);
						success = true;
					}
				}
			}
	...
	private boolean tryGrowRoom(Vec3i pos, PathSegment prev, Direction dir, int maxWidth, int maxLength, int preferredArea) {
	...
	private void placeDoor(Vec3i pos, Direction facing, PathSegment segment) {
	...
	@Deprecated
	private PathSegment addRoomSegment(Vec3i pos, Direction dir, int l, int w_l, int w_r, int maxL, int maxW_l, int maxW_r ) {
	...
	private int countFreeSpaceInDir(Vec3i pos, Direction dir) {
	...
	private int getRandomDir(int dir) {
		...
		if (f < chanceStraight) {
			...
			return Direction.getHorizontal(dir).rotateY().getHorizontalIndex();
		}else {
			return Direction.getHorizontal(dir).rotateYCCW().getHorizontalIndex();
		}	
	}
	...
	public class PathSegment {
		...
		public Vec3i getNextPos(int dir) {
			Vec3i offset = Direction.getHorizontal(dir).getDirectionVec();
			return new Vec3i(x+offset.getX(), y+offset.getY(), z+offset.getZ());
		}
		
		public Vec3i getNextPos(Direction facing) {
			Vec3i offset = facing.getDirectionVec();
			return new Vec3i(x+offset.getX(), y+offset.getY(), z+offset.getZ());
		}
		...
		public boolean connectionAt(Direction facing) {
		...
		public void setConnection(Direction facing, boolean connected) {
			int i = ((facing.getHorizontalIndex()+2) % 4) * 2 + 1;
			pattern[i] = connected;
...
//							preset.getSegment(SegmentType.PILLARS, -1, 0, this.sY, above, below, world.rand.nextInt()).placeSegment(world, px, py, pz, 0);		
	...
	public void generateNPCSpawners(World world, int posX, int posY, int posZ, IDungeonPreset preset) {
		...
		for (PathSegment seg : spawnPositions) {
			...
			if(pos.getY()>=1) {
				BlockState state = TGBlocks.MONSTER_SPAWNER.getDefaultState();
