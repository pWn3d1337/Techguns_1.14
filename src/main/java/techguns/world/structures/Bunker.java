...
public class Bunker extends WorldgenStructure {
	...
	public void setBlocks(World world, int posX, int posY, int posZ, int sizeX,
		...
		if (type == 2 || type == 3) {
			//Lamp
			world.setBlockState(p.setPos(posX+xoffset-dx, posY+1, posZ+zoffset-dz), BlockRotator.getWithFacing(lamps.getState(), Direction.UP), 2);
//			if (type == 2) {
...
//			}else {
			...
			//Lamp
			world.setBlockState(p.setPos(posX+(sizeX/2), posY+1, posZ+(sizeZ/2)), BlockRotator.getWithFacing(lamps.getState(), Direction.UP), 2);
			...
			for (int z=0; z <sizeZ; z++) {
				...
				if (chest==null){
					...
					if(tile !=null && tile instanceof ChestTileEntity) {
						((ChestTileEntity)tile).setLootTable(CHEST_LOOT, world.rand.nextLong());
					}
