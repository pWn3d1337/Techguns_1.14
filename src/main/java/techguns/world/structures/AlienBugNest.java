...
//	}
			...
			for (int y = y1-r; y < y2+r; y++) {
				for (int z = z1-r; z < z2+r; z++) {
					...
					if (distance < radius  && TGBlocks.SLIMY_LADDER.canPlaceBlockAt(world, p)) {
						world.setBlockState(p, TGBlocks.SLIMY_LADDER.getStateForPlacement(world, p, Direction.NORTH, 0, 0, 0, 0, null), 2);
	...
	static class MBlockSlime extends MBlock {
		...
		public void setBlock(World w, MutableBlockPos pos, int rotation, EnumLootType loottype, BiomeColorType biome) {
			...
			placeSlimeAt(w, new MutableBlockPos(pos).move(Direction.NORTH));
			placeSlimeAt(w, new MutableBlockPos(pos).move(Direction.EAST));
			placeSlimeAt(w, new MutableBlockPos(pos).move(Direction.SOUTH));
			placeSlimeAt(w, new MutableBlockPos(pos).move(Direction.WEST));
		}
		...
		private void placeSlimeAt(World world, MutableBlockPos pos) {
			if (world.isAirBlock(pos) && TGBlocks.SLIMY_LADDER.canPlaceBlockAt(world, pos)) { //TGBlocks.bugnestslimy.canPlaceBlockAt(world, x, y, z)) {						
				...
				world.setBlockState(pos, TGBlocks.SLIMY_LADDER.getStateForPlacement(world, pos, Direction.NORTH, 0, 0, 0, 0, null), 2);
				//world.setBlockMetadataWithNotify(x,y,z,((BlockBugSlime)TGBlocks.bugnestslimy).getDirectionMeta(world, x, y, z), 2);
			}
		}
