...
public class BlockUtils {
	...
	public static String getBlockStateVariantString(BlockState state) {
	...
	protected static boolean checkMiningLevels(World world, PlayerEntity ply, BlockPos b, GenericGunMeleeCharge miningtool, ItemStack stack) {
	...
	public static List<BlockPos> getBlockPlaneAroundAxisForMining(World world, PlayerEntity ply, BlockPos center, Direction.Axis axis, int radius, boolean includeCenter, @Nullable GenericGunMeleeCharge miningtool, ItemStack stack){
	...
	public static int getValidSpawnYWater(World world, int x, int z, int sizeX, int sizeZ, int heightDiffLimit){
 	...
 	public static int getHeightValueNoTreesIgnoreLiquid(World w, int x, int z){
 		...
 		BlockState bs = w.getBlockState(p.setPos(x, y, z));
 		Block b = bs.getBlock();
 	...
 	public static int getHeightValueTopBlockAll(World w, int xCoord, int zCoord)
    {
        ...
        for (zCoord &= 15; k > 0; --k)
        {
            BlockState block = chunk.getBlockState(xCoord, k, zCoord);
    ...
    private static int getTopSolidOrLiquidBlock(World w, int posx, int posz)
    {
        ...
        for (posz &= 15; k > 0; --k)
        {
            BlockState block = chunk.getBlockState(posx, k, posz);
 	...
 	public static void removeJunkInArea(World world, int posX, int posZ, int sizeX, int sizeZ) {
		...
		for (int x = 0; x < sizeX; x++) {
			for (int z = 0; z <sizeZ; z++) {
				...
				boolean stop = false;
				for (; y > 0 && !stop; y--) {
					BlockState bs = world.getBlockState(pos.setPos(posX+x, y, posZ+z));
	...
	public static void fillBlocks(World world, MutableBlockPos start, int sizeX, int sizeY, int sizeZ, BlockState block) {
		int x = start.getX();
		int y = start.getY();
		int z = start.getZ();
	...
	public static void fillBlocksHollow(World world, MutableBlockPos start, int sizeX, int sizeY, int sizeZ, BlockState block) {
		int x = start.getX();
		int y = start.getY();
		int z = start.getZ();
	...
	public static BiomeColorType getBiomeType(World world, int posX, int posZ) {
 	...
 	private static boolean isGroundBlock(BlockState b) {
 	...
 	private static void adjustHeightAtPos(World world, int x, int z, int y, MutableBlockPos p, int newY) {
		if (newY  > y) {
			BlockState b = world.getBlockState(p.setPos(x, y, z));
			if (!isGroundBlock(b)) {
...
//	                }
   ...
   * @return
   */
	public static Direction rotationToFacing(int rot) {
		switch(rot) {
		case 0:
	 ...
	 * @return
	 */
	public static int facingToRotation(Direction facing) {
		switch (facing) {
		case WEST:
			return 0;
		case SOUTH:
