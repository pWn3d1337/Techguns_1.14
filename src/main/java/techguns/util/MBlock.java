...
public class MBlock implements Serializable {
	...
	protected transient BlockState state;
	...
	public MBlock(BlockState state) {
		this(state,false);
	}
	
	public MBlock(BlockState state, boolean hasTileEnt) {
	...
	public void setBlock(World w, MutableBlockPos pos, int rotation, EnumLootType loottype, BiomeColorType biome) {
		if(pos.getY()>=1) {
			BlockState targetState = BlockRotator.getRotatedHorizontal(state, rotation);
