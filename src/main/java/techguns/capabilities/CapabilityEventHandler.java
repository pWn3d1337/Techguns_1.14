...
public class CapabilityEventHandler {
	...
	public static void attachCapabilities(final AttachCapabilitiesEvent<Entity> event) {
		...
		if (event.getObject() instanceof INPCTechgunsShooter) {
			...
			if( event.getObject().world.isRemote) {
				cap = new TGExtendedPlayerClient((PlayerEntity) event.getObject());
			} else {
				cap = new TGExtendedPlayer((PlayerEntity) event.getObject());
			}
	...
	public static void attachCapabilitiesClient(final AttachCapabilitiesEvent<Entity> event) {
		if (event.getObject() instanceof LivingEntity) {
			final TGDeathTypeCap cap = new TGDeathTypeCap((LivingEntity) event.getObject());
