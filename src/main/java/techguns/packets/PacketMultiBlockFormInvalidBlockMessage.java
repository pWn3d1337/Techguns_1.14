...
public class PacketMultiBlockFormInvalidBlockMessage implements IMessage {
	...
	public static class Handler extends HandlerTemplate<PacketMultiBlockFormInvalidBlockMessage>{
		...
		protected void handle(PacketMultiBlockFormInvalidBlockMessage message, MessageContext ctx) {
			PlayerEntity player = TGPackets.getPlayerFromContext(ctx);
			BlockPos p = message.pos;
			switch(message.type) {
			case MSG_TYPE_ORE_DRILL_ENGINE_ERROR:
				player.sendStatusMessage(new StringTextComponent(TextUtil.trans("techguns.multiblock.oredrill.invalid.engines")), true);
				break;
			case MSG_TYPE_ORE_DRILL_FRAME_ERROR:
				player.sendStatusMessage(new StringTextComponent(TextUtil.trans("techguns.multiblock.oredrill.invalid.frame")), true);
				break;
			case MSG_TYPE_ORE_DRILL_SCAFFOLD_ERROR:
				player.sendStatusMessage(new StringTextComponent(TextUtil.trans("techguns.multiblock.oredrill.invalid.scaffold")), true);
				break;
			case MSG_TYPE_ORE_DRILL_AIR_ERROR:
				player.sendStatusMessage(new StringTextComponent(TextUtil.trans("techguns.multiblock.oredrill.invalid.air")), true);
				break;
			...
			case MSG_TYPE_ORE_DRILL_CONTROLLER_PLACEMENT_ERROR:
				player.sendStatusMessage(new StringTextComponent(TextUtil.trans("techguns.msg.error.oredrill.controllernext")), true);
				break;
			case MSG_TYPE_ORE_DRILL_CONTROLLER_ALONE:
				player.sendStatusMessage(new StringTextComponent(TextUtil.trans("techguns.msg.error.oredrill.controlleralone")), true);
				break;
			case MSG_TYPE_MULTIBLOCK_ERROR:
				player.sendStatusMessage(new StringTextComponent(TextUtil.trans("techguns.msg.error.multiblockinvalid")), true);
				break;
			case MSG_TYPE_ROD_SIZE:
				player.sendStatusMessage(new StringTextComponent(TextUtil.trans("techguns.msg.error.oredrill.rodsize")), true);
				break;
			default:
				player.sendStatusMessage(new StringTextComponent(TextUtil.trans("techguns.multiblock.invalidblock")+": "+p.getX()+ ", "+p.getY()+", "+p.getZ()), false);
			}
