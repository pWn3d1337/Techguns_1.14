...
public class Keybinds {
    ...
    public static void onKeyInput(InputEvent.KeyInputEvent event) {
        ...
        if(Keybinds.plusX.isPressed()){
        	Keybinds.X+=D;
			Minecraft.getMinecraft().player.sendMessage(new StringTextComponent("X:"+Keybinds.X));
        }
        ...
        if(Keybinds.plusY.isPressed()){
        	Keybinds.Y+=D;
			Minecraft.getMinecraft().player.sendMessage(new StringTextComponent("Y:"+Keybinds.Y));
        }
        ...
        if(Keybinds.plusZ.isPressed()){
        	Keybinds.Z+=D;
			Minecraft.getMinecraft().player.sendMessage(new StringTextComponent("Z:"+Keybinds.Z));
        }
        ...
        if(Keybinds.showXYZ.isPressed()){
        	Minecraft.getMinecraft().player.sendMessage(new StringTextComponent("XYZ: "+Keybinds.X+", "+Keybinds.Y+", "+Keybinds.Z));
        }
        if(Keybinds.reloadFX.isPressed()) {
        	Minecraft.getMinecraft().player.sendMessage(new StringTextComponent("Reload FX..."));
        	TGFX.loadFXList();
        	Minecraft.getMinecraft().player.sendMessage(new StringTextComponent("Reload FX... Done!"));
        }
        if(Keybinds.resetXYZ.isPressed()) {
        	Keybinds.Z=0f;
        	Keybinds.Y=0f;
        	Keybinds.X=0f;
			Minecraft.getMinecraft().player.sendMessage(new StringTextComponent("Reset XYZ"));
        }
    }
