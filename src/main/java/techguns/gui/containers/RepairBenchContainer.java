...
public class RepairBenchContainer extends OwnedTileContainer {
	...
	public RepairBenchContainer(PlayerInventory player, RepairBenchTileEnt ent) {
		super(player, ent);
		...
		if(slot.getHasStack()){
			...
			if (!stack.isEmpty()){
				...
				if(slotid<9){
					...
					if(stack1.getItem() instanceof ArmorItem){
						int type=getArmorIntFromEntityEquipmentSlot(((ArmorItem)stack1.getItem()).armorType);
