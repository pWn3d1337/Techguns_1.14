...
public class TGPlayerInventoryContainer extends Container {
	...
	public TGPlayerInventoryContainer(PlayerEntity player){
		...
		//Armor
		for (int k = 0; k < 4; ++k) {
			EquipmentSlotType entityequipmentslot = VALID_EQUIPMENT_SLOTS[k];
	...
	public ItemStack transferStackInSlot(PlayerEntity player, int id) {
	      ...
	      ItemStack itemstack = ItemStack.EMPTY;
	        ...
	        if (slot != null && slot.getHasStack())
	        {
	            ...
	            //Try merge Armor
	            else if (itemstack.getItem() instanceof ArmorItem && !((Slot)this.inventorySlots.get(TGPlayerInventory.NUMSLOTS + ((ArmorItem)itemstack.getItem()).armorType.getIndex())).getHasStack())
	            {
	                int j = TGPlayerInventory.NUMSLOTS + ((ArmorItem)itemstack.getItem()).armorType.getIndex();
