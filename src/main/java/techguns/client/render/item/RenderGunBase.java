...
public class RenderGunBase extends RenderItemBase {
	...
	protected static ITGShooterValues getShooterValues(LivingEntity ent) {
		...
		if (ent != null) {
			if (ent instanceof PlayerEntity) {
				values = TGExtendedPlayer.get((PlayerEntity) ent);
			} else {
	...
	@Override
	public void renderItem(TransformType transform, ItemStack stack, LivingEntity entityIn, boolean leftHand) {
		...
		if (entityIn!=null){
			...
			isOffhand = ((!leftHand) && entityIn.getPrimaryHand() == HandSide.LEFT) || ((leftHand) && entityIn.getPrimaryHand() == HandSide.RIGHT);
...
//				System.out.println("Lefthand = "+leftHand+" isOffhand = "+isOffhand);
	...
	public void applyAnimForParticles(LivingEntity entity, float reloadProgress, TransformType transform, boolean sneaking, boolean isOffhand) {
		GlStateManager.pushMatrix();
	...
	protected void transformThirdPerson(LivingEntity ent, float fireProgress, float reloadProgress, boolean left) {
