...
public class RenderItemBase implements IItemRenderer {
	...
	protected void setBaseScale(LivingEntity entity, TransformType transform) {
	...
	protected void renderItemParticles(LivingEntity ent, TransformType transform, float ptt) {
		...
		if (transform == TransformType.FIRST_PERSON_LEFT_HAND || transform == TransformType.THIRD_PERSON_LEFT_HAND) {
			...
			if(ent.getPrimaryHand() == HandSide.LEFT) {
				hand = Hand.OFF_HAND;
			}
		...
		if(ent instanceof PlayerEntity) {
			if(hand== Hand.MAIN_HAND) {
				particles = TGExtendedPlayerClient.get((PlayerEntity) ent).getEntityParticlesMH();
			} else {
				particles = TGExtendedPlayerClient.get((PlayerEntity) ent).getEntityParticlesOH();
			}
...
//	        float f5 = MathHelper.cos(entityIn.rotationPitch * 0.017453292F - (float) angles.x);
	...
	public static Vec3d getTransformAngles(LivingEntity entity, HandSide hand, float ptt) {
