...
//        p2 = new Vec3d((x2+x3)*0.5, (double)( + rotZ * fscale), (z2+x3)*0.5);
        ...
        if (prev == null) {
			...
			Vector3d v_view = Minecraft.getInstance().player.getLook(partialTickTime);

    		Vector3d v_prev = new Vector3d(prev.posX, prev.posY, prev.posZ).subtract(Minecraft.getInstance().player.getEyePosition(partialTickTime));
...
//            System.out.printf("PrevP1: X=%.3f,  Y=%.3f, Z=%.3f\n", prev.pos1.x, prev.pos1.y, prev.pos1.z);
    		...
    		Vector3d v_cross = v_view.crossProduct(v_dir).normalize();
	...
	//        }	        			
			...
			buffer.pos(p1.x, p1.y, p1.z).tex((float) ua, (float) va).color(this.particleRed, this.particleGreen, this.particleBlue, this.particleAlpha).lightmap(0, 240).normal(0.0f, 1.0f, 0.0f).endVertex();
			buffer.pos(p2.x, p2.y, p2.z).tex((float) ub, (float) vb).color(this.particleRed, this.particleGreen, this.particleBlue, this.particleAlpha).lightmap(0, 240).normal(0.0f, 1.0f, 0.0f).endVertex();
			buffer.pos(p3.x, p3.y, p3.z).tex((float) uc, (float) vc).color(prev.particleRed, prev.particleGreen, prev.particleBlue, prev.particleAlpha).lightmap(0, 240).normal(0.0f, 1.0f, 0.0f).endVertex();
			buffer.pos(p4.x, p4.y, p4.z).tex((float) ud, (float) vd).color(prev.particleRed, prev.particleGreen, prev.particleBlue, prev.particleAlpha).lightmap(0, 240).normal(0.0f, 1.0f, 0.0f).endVertex();
