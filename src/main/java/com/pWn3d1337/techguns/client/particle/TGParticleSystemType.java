...
public class TGParticleSystemType extends TGFXType{
	TGRenderHelper.TGRenderType renderType = TGRenderHelper.TGRenderType.SOLID; //Type of Transparency
	...
	public static IVelocityType VEL_NONE = new IVelocityType(){
		@Override
		public Vector3d getVelocity(TGParticleSystem sys, float... params) {
...
//				}
	...
	public static IVolumeType VOL_POINT = new IVolumeType() {
		...
		@Override
		public Vector3d getPosition(TGParticleSystem sys, DirResult dir, int i, int count) {
			return new Vector3d(0,0,0);
		}
	...
	public static IVolumeType VOL_CYLINDER = new IVolumeType() {
		...
		public Vector3d getPosition(TGParticleSystem sys, DirResult dir, int i, int count) {
			...
			if (sys.type.volumeData.length >= 2) {
				...
				Vector3d direction = new Vector3d(1.0, 0.0, 0.0).rotateYaw(angle);
	...
	public static IVolumeType VOL_CYLINDER2 = new IVolumeType() { //Tilted cylinder
		...
		public Vector3d getPosition(TGParticleSystem sys, DirResult dir, int i, int count) {
			...
			if (sys.type.volumeData.length >= 2) {
				...
				Vector3d direction = new Vector3d(0.0, 1.0, 0.0).rotatePitch(angle); //.rotateYaw((float)(Math.PI*0.5d));
				dir.setValues((float)direction.x, (float)direction.y, (float)direction.z);
	...
	public static IVolumeType VOL_TRAIL = new IVolumeType() {
		...
		public Vector3d getPosition(TGParticleSystem sys, DirResult dir, int i, int count) {
			...
			if (sys.type.volumeData.length >= 1) {
				...
				if (ent==null){
					return new Vector3d(sys.posX(), sys.posY(),sys.posZ());
				} else {
					
					Vector3d motion = new Vector3d(ent.getMotion().x, ent.getMotion().y, ent.getMotion().z).normalize();
