...
import com.pWn3d1337.techguns.api.capabilities.AttackTime;
import com.pWn3d1337.techguns.api.capabilities.ITGExtendedPlayer;
...
public class TGExtendedPlayer implements ITGExtendedPlayer {
	...
	public static final DataParameter<ItemStack> DATA_FACE_SLOT = EntityDataManager.<ItemStack>createKey(PlayerEntity.class, DataSerializers.ITEMSTACK);
	public static final DataParameter<ItemStack> DATA_BACK_SLOT = EntityDataManager.<ItemStack>createKey(PlayerEntity.class, DataSerializers.ITEMSTACK);
	public static final DataParameter<ItemStack> DATA_HAND_SLOT = EntityDataManager.<ItemStack>createKey(PlayerEntity.class, DataSerializers.ITEMSTACK);
	
	public static final DataParameter<Boolean> DATA_FLAG_CHARGING_WEAPON = EntityDataManager.<Boolean>createKey(PlayerEntity.class, DataSerializers.BOOLEAN);
	...
	public TGExtendedPlayer(PlayerEntity entity) {
		this.entity = entity;
	...
	public static TGExtendedPlayer get(PlayerEntity ply){
	...
	@Override
	public void saveToNBT(CompoundNBT tags) {
	...
	@Override
	public void loadFromNBT(CompoundNBT tags) {
	...
	public void addDropsToList(PlayerEntity player, List<ItemEntity> list ){
		if(!player.world.isRemote){
			if (!player.world.getGameRules().getBoolean("keepInventory"))
	        {
	            //this.inventory.dropAllItems();
	            ...
	            for (i = 0; i < this.tg_inventory.inventory.size(); ++i)
	            {
	                ...
	                {
	                	//System.out.println("Dropping "+TG_inventory.inventory[i].getDisplayName()+" x"+TG_inventory.inventory[i].stackSize);
	                    ItemEntity item = player.dropItem(this.tg_inventory.inventory.get(i), true, false);
	                    if(item!=null) {
	                    	list.add(item);
	                    }
	                    this.tg_inventory.inventory.set(i, ItemStack.EMPTY);
